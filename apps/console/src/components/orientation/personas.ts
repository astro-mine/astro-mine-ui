...
/**
 * What has no page, and what does it instead.
 *
 * **The list `ui#34` asserts against the navigation table**, so it cannot rot as pages land: if
 * something here ever gains a route, the test fails and the entry has to go. That is the mechanism
 * behind `ui.md` §5's claim that *nothing is GUI-unreachable by construction* — a capability with
 * no GUI has to be discoverable *from* the GUI, or the claim has nowhere to land.
 */
